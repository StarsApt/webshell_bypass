...
function enjsp($content,$comment) {
  ...
  //echo strrpos($content,"<%"."<br>";
    ...
    $var=''; 
    $var .='<%@page'. $need_1.'%>'."\r";
    $var .='<%'.'/*'.' '.$comment.' '.'*/'.$need_2.'%>'."\r";
    $var .='<%'.$need_3.'%>'."\r";
    $string_code='';
    $re_code='\u';
    $rand=randzore(10);
    $string_code .=$re_code.$rand;
   // echo $string_code;
   //$content=$var;
    $content=str_ireplace('\u', $string_code, $var);
    return $content;
}
 ...
 * @return string
 */
function enjsp_file($file, $target_file,$comment) {
  $content = file_get_contents($file);
  $content = enjsp($content,$comment);
  if ($target_file) {
